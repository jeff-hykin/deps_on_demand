...
def _write_imports_init(
    ...
    lines.append("")
    lines.append(f"__all__ = {sorted(symbol for _, symbol in modules)!r}")
    lines.append("")
    lines.append("_EXTRA_HINT = {0!r}".format(f"pip install .[{extra_name}]"))
    lines.append("")
    lines.append("def _missing_dep_error(modname: str) -> ModuleNotFoundError:")
    lines.append("    msg = (")
    lines.append('        f\"Optional dependency {modname!r} is required for this feature. \"')
    lines.append("        f\"Install it with: {_EXTRA_HINT}\"")
    lines.append("    )")
    lines.append("    return ModuleNotFoundError(msg)")
    lines.append("")
    lines.append("")
    lines.append("class _MissingTypeMeta(type):")
    lines.append("    def __call__(cls, *args: Any, **kwargs: Any) -> Any:  # type: ignore[override]")
    ...
    lines.append("")
    lines.append("")
    lines.append("def _load_summaries() -> Dict[str, Dict[str, Any]]:")
    lines.append("    base = Path(__file__).parent")
    lines.append("    summaries: Dict[str, Dict[str, Any]] = {}")
    ...
    lines.append("        summaries[name] = data")
    lines.append("    return summaries")
    lines.append("")
    lines.append("")
    lines.append("_SUMMARIES = _load_summaries()")
    lines.append("")
    for import_name, symbol_name in modules:
        lines.append(f"{symbol_name} = _LazyModuleProxy(_SUMMARIES[{symbol_name!r}]['module'], _SUMMARIES[{symbol_name!r}]['summary'])")
    lines.append("")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for import_name, symbol_name in modules:
        ...
        with out_path.open("w", encoding="utf-8") as f:
            json.dump({"module": import_name, "summary": summary}, f, indent=2, sort_keys=True)
            f.write("\n")
        written.append((import_name, symbol_name))

    _write_imports_init(imports_dir, written, args.extra)
